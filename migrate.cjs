...
async function downloadImage(imgUrl, postSlug) {
  ...
  try {
    ...
    if (!res.ok) { console.error(`Image fetch fail ${fullUrl}`); return null; }
...
function extractAuthor($) {
  ...
  if (byline.length) {
    const t = byline.text().trim();
    return t || 'SpeedwayWorld WebTeam';
  }
...
function extractContent($) {
  ...
  html = html.replace(/<div class="sidebar-content[^"]*">[\s\S]*?<\/div>/g, '');
